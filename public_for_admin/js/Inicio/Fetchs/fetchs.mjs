...
async function create_activities_admin(activityData) {
    try {
        const response = await fetch('/create_activities_admin', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify(activityData) // activityData debe ser un objeto o un array
        });

        const result = await response.json();
        if (!response.ok) {
            throw new Error(result.message || 'Error al crear actividad');
        }
        alert(`Actividad creada`);
        return result;
    } catch (err) {
        console.error('Error al enviar la actividad:', err);
        alert(`Error al enviar la actividad: ${err.message}`);
    }
}
...
export {getCookie, get_all_photo,last_conexion,get_all_messages,upload_message,
    ...
    get_all_activities_user,deleteActivityAndFile,downloadFile, create_activities_admin}